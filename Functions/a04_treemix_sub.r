...
plot_tree_internal = function(d, e, o = NA, cex = 1, disp = 0.005, plus = 0.005, arrow = 0.05, ybar = 0.01, scale = T, mbar = T, mse = 0.01, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1){
	...
	if (scale){
		print (paste("mse", mse))
		lines(c(0, mse*10), c(ybar-0.04, ybar-0.04))
		# text( 0, ybar - 0.04, lab = "10 s.e.", adj = 0, cex  = 0.8)
		lines( c(0, 0), c( ybar - 0.005-0.04, ybar+0.005-0.04))
		lines( c(mse*10, mse*10), c(ybar- 0.005-0.04, ybar+ 0.005-0.04))
	}
        if (mbar){
                mcols = rev( heat.colors(150) )
                ...
                rect( rep(xma-0.003, 100), ymi+(0:99)*w, rep(xma-0.002, 100), ymi+(1:100)*w, col = mcols, border = mcols)
                text(xma-0.0018, ymi, lab = "0", adj = 0, cex = 0.7)
		if ( mw >0.5){ text(xma+disp, yma, lab = "1", adj = 0, cex = 0.7)}
                else{
			text(xma-0.0018, yma, lab = "0.5", adj = 0, cex =0.7)
		}
		# text(xma-0.003, yma+0.06, lab = "Migration", adj = 0 , cex = 0.6)
		# text(xma-0.003, yma+0.03, lab = "weight", adj = 0 , cex = 0.6)
        }	
}
